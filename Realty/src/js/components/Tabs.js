...
export default React.createClass({
    ...
    render() {
        ...
        for (let i=0; i<this.props.children.length; i++)  {
            ...
            tabHeaders.push(<TabHeader label={tab.props.label} index={i} active={i===this.state.activeTabIndex} onClick={this.clickHandler} key={i}/>);
            tabs.push(<Tab active={i===this.state.activeTabIndex} key={i}>{tab.props.children}</Tab>);
        };
